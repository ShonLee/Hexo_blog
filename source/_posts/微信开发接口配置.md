...
3. 到这里,SAE的配置就完成了,下面就需要把我们编写好的链接微信的测试程序上传到SAE,进行链接测试,我是通过Xshell链接SAE来上传我的测试文件的,[移步这里](https://shonlee.github.io/2017/04/11/Xshell%E9%80%9A%E8%BF%87SSH%E9%93%BE%E6%8E%A5SAE/)

<font color="red">我把测试链接程序打成了WAR包上传的,项目打WAR包方法请找度娘</font>

下面是创建连接测试的接口程序:

>[引用柳峰的博客](http://blog.csdn.net/lyq8479/article/details/8944988#)

创建一个Java Web工程，并新建一个能够处理请求的Servlet，命名任意，我在这里将其命名为`org.liufeng.course.servlet.CoreServlet`，代码如下：
...
在doGet方法中调用了`org.liufeng.course.util.SignUtil.checkSignature`方法，`SignUtil.Java`的实现如下：
...
最后再来看一下web.xml中，`CoreServlet`是怎么配置的，web.xml中的配置代码如下
